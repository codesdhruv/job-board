...
- `src/layouts/AdminLayout.astro` — **F-15 (updated 2026-06-14).** Bare HTML shell with `noindex` meta; accepts `title` prop. Now includes a persistent left sidebar (220px, white bg, `#ebebeb` right border) rendered on all admin pages **except** `/admin/login` (path-gated via `Astro.url.pathname`). Sidebar shows site name + 4 nav links: Dashboard (`/admin/dashboard`), All Jobs (`/admin/jobs`), Add New Job (`/admin/jobs/new`), Logout (`/admin/logout`). Active link uses `aria-current="page"` + `background:#f4f4f4` + `font-weight:600`. Body is a flex container; `<main>` takes `flex:1` with `padding:32px 36px`. Inline styles only, design-token palette.
...
- `src/pages/admin/login.astro` — **F-14 (built).** SSR page at `/admin/login`. Reads `ADMIN_SECRET` via `(env as any).ADMIN_SECRET` (throws if missing). Short-circuits already-logged-in requests: any `admin_session` cookie is passed to `verifySession()`; on valid payload, redirects to `/admin/dashboard`. On POST: parses `formData()` for `username`/`password`, queries `SELECT id, username, password_hash FROM admins WHERE username = ? LIMIT 1`, calls `verifyPassword()` from `lib/auth.ts` (**PBKDF2, not bcryptjs** — the F-14 spec line 859 says bcrypt but the project deliberately uses PBKDF2 via Web Crypto; see `lib/auth.ts:43-58`). Success → mint 8-hour token via `signSession({adminId, username, expiresAt}, secret)`, set cookie `admin_session` with `httpOnly: true, secure: true, sameSite: 'strict', path: '/', expires: new Date(expiresAt)`, redirect to `/admin/dashboard`. Failure (missing field, unknown username, bad password) → re-render the form with a single generic `Invalid username or password` alert; submitted username pre-filled, password always blank. Uses `AdminLayout` (which emits `noindex, nofollow`). Inline styles only — design-token palette. **Out of scope for this prompt:** rate limiting (5 fails/IP/15 min via KV — spec line 858). Verified 2026-06-14: GET 200 with form; POST wrong password → 200 with alert + preserved username + no cookie; POST correct credentials → 302 Location `/admin/dashboard` with `Set-Cookie: admin_session=…; HttpOnly; Secure; SameSite=Strict; Expires=…`; GET with valid cookie → 302 to dashboard. **Note:** Astro v6's built-in CSRF guard rejects cross-origin POSTs (403 `"Cross-site POST form submissions are forbidden"`) — browsers always send `Origin`, so this is correct; curl tests must add `-H "Origin: http://127.0.0.1:8789"`. See `.claude/plans/21-Admin-Login-F14.md`.
- `src/pages/admin/dashboard.astro` — **F-15 (built 2026-06-14).** SSR page at `/admin/dashboard`. Auth enforced by middleware (F-23) — no per-page session check needed. Runs 6 D1 queries in a single `Promise.all`: 4 COUNT queries (live jobs, expiring in 7 days, drafts, expired) + "expiring soon" list (jobs with `last_date BETWEEN DATE('now') AND DATE('now', '+7 days')` ordered by `last_date ASC` LEFT JOINed with departments) + "recently added" list (last 5 jobs LEFT JOINed with `job_types` and `states`, `ORDER BY created_at DESC`). Renders: 4 metric cards in a 4-col CSS grid (live=blue `#0070f3`, expiring=orange `#f5a623`, drafts=grey `#888888`, expired=red `#ee0000`); "Expiring soon" table (Title | Department | Last date in red | Edit + Delete links); "Recently added" table (Title | Type | State | Posted | Edit + Delete links). Empty-state text shown instead of empty tables. `formatDate()` handles both `YYYY-MM-DD` and D1's `YYYY-MM-DD HH:MM:SS` datetime format by replacing space with `T` before `new Date()`. Edit/Delete links point to `/admin/jobs/{id}/edit` and `/admin/jobs/{id}/delete` (now live — F-18/F-19 built 2026-06-14). Inline styles only, design tokens. Verified 2026-06-14: metric counts match D1 seed data (2 live, 1 expiring, 0 drafts, 0 expired); expiring table shows UPSC Engineering Services 2026; recently added shows 2 seeded rows with correct dates. See `.claude/plans/22-Admin-Dashboard-F15.md`.
- `src/pages/admin/jobs/index.astro` — **F-16 (built 2026-06-14).** SSR page at `/admin/jobs`. Auth enforced by middleware (F-23). Two parallel D1 queries: `COUNT(*) FROM jobs` (no filter — all statuses) + full job list `ORDER BY created_at DESC LIMIT 50 OFFSET ?` LEFT JOINed with `job_types` and `states`. Status logic per row: `is_published=0` → Draft (amber `#f5a623`); `last_date < today` → Expired (red `#ee0000`); else → Live (green `#22a565`). Table columns: ID | Title | Type | State | Last Date | Status | Edit | Delete. Edit links to `/admin/jobs/{id}/edit`; Delete is an inline `<form method="POST" action="/admin/jobs/{id}/delete">` with `onsubmit="return confirm('Delete this job?')"` (button styled as red text link). Flash banners from `?success=1` / `?updated=1` / `?deleted=1` query params (green alert bar). Inline pagination when `totalPages > 1`: centered prev/next + up to 5 page numbers windowed on current page. `+ Add New Job` button (dark pill) in header links to `/admin/jobs/new`. `formatDate()` inline (same as dashboard). Inline styles, design-token palette. Verified 2026-06-14: 200 with session cookie, 302→login without; both seeded jobs render Live; flash `?success=1` shows "Job posted successfully."; sidebar marks "All Jobs" active. See `.claude/plans/23-Admin-Jobs-List-F16.md`.
- `src/pages/admin/jobs/new.astro` — **F-17 (built 2026-06-14).** SSR page at `/admin/jobs/new`. Auth enforced by middleware (F-23). On GET: fetches job_types, all states (split into All India / States / UTs), categories, and departments via parallel D1 queries; renders a 6-section form (Basic Info, Location, Job Details, Apply, Description, Publishing). State `<select>` mirrors the public filter's optgroup structure: "All India (Central Govt)" first, then `<optgroup label="States">` and `<optgroup label="Union Territories">`. Slug field auto-generated from title via `<script is:inline>` slugify (lowercase, strip special chars, spaces→hyphens, max 80 chars); admin edits lock out auto-generation via a `slugTouched` flag. Status defaults to Published (checkbox pre-checked). On POST: server-side validation — title required ≤200 chars; job_type_id required + valid FK; last_date required and ≥ today; state_id required when selected type slug is `government` or `psu`; at least one of apply_url or apply_email required; apply_url validated via `new URL()`; apply_email validated via regex; slug derived from title if blank, then auto-suffixed (`-2`, `-3`, …) via `makeUniqueSlug()` until unique. On success: INSERT into `jobs`, `invalidateAllListingCaches(kv)`, redirect to `/admin/jobs?success=1` (flash banner wired in F-16). On error: form re-rendered with per-field `<span style="color:#ee0000">` errors and all submitted values preserved. Inline styles, design-token palette. Verified 2026-06-14: empty form → 5 errors (title, job_type, last_date, apply, slug); govt+no-state → "State is required for Government jobs."; valid govt POST → 302 `/admin/jobs?success=1`; job appears on public `/jobs` after KV invalidation. See `.claude/plans/24-Post-New-Job-F17.md`.
- `src/pages/admin/jobs/[id]/edit.astro` — **F-18 (built 2026-06-14).** SSR page at `/admin/jobs/[id]/edit`. Auth enforced by middleware. GET: parses `id` from params (404 on NaN or missing row); fetches existing job via `SELECT * FROM jobs WHERE id = ? LIMIT 1`; loads dropdown data (job_types, states, categories, departments) via parallel D1 queries; seeds `values` object from the existing job row so all fields are pre-populated. POST: identical validation to F-17 (title ≤200, job_type_id required+valid FK, last_date ≥ today, state required for govt/psu, at least one of apply_url/apply_email, URL/email format checks, slug format check). Slug uniqueness uses `makeUniqueSlugExcluding(slug, jobId)` — excludes the current job's own ID so unchanged slugs never trigger a conflict; only called when slug differs from `originalSlug`. On success: `UPDATE jobs SET … updated_at = CURRENT_TIMESTAMP WHERE id = ?`, `invalidateAllListingCaches(kv)`, redirect to `/admin/jobs?updated=1` (flash: "Job updated." — already wired in F-16). On error: form re-rendered with per-field errors and all submitted values preserved. **Slug change warning:** slug input has `data-original={originalSlug}`; inline `<script is:inline>` shows `#slug_warning` span ("⚠ Changing the slug will break existing links.") in warning orange (`#f5a623`) whenever the current value differs from the original — hides it again if restored. State asterisk toggle logic retained from F-17. Inline styles, design-token palette. Build verified 2026-06-14.
- `src/pages/admin/jobs/[id]/delete.astro` — **F-19 (built 2026-06-14).** POST-only endpoint at `/admin/jobs/[id]/delete`. Auth enforced by middleware. GET returns `405 Method Not Allowed` with `Allow: POST` header. Parses `id` (404 on NaN). `DELETE FROM jobs WHERE id = ?`; `invalidateAllListingCaches(kv)`; redirect to `/admin/jobs?deleted=1` (flash: "Job deleted." — already wired in F-16). No HTML body — route exists only to handle the inline delete `<form method="POST">` in F-16. Hard delete only (no soft-delete); idempotent (SQLite DELETE is no-op on missing row). Build verified 2026-06-14.
...
src/
  pages/
    ...
    admin/
      ...
      dashboard.astro           ✓ F-15 — 4 metric counts + expiring soon table + recently added table
      jobs/
        index.astro             ✓ F-16 — All jobs table (50/page, status badges, flash messages, inline pagination)
        new.astro               ✓ F-17 — Post new job (6-section form, server validation, auto-slug, KV invalidation)
        [id]/edit.astro         ✓ F-18 — Edit job (pre-populated form, slug-change warning, UPDATE + KV invalidation)
        [id]/delete.astro       ✓ F-19 — Delete job (POST-only, hard delete, KV invalidation, 405 on GET)
  components/
...
- **F-15** ✓ Admin dashboard — `src/pages/admin/dashboard.astro`. 6 parallel D1 queries: 4 COUNT metrics (live/expiring/drafts/expired) + expiring-soon list + recently-added (last 5). Metric cards colour-coded by status. Both tables have Edit/Delete links (404 until F-18/F-19). Sidebar added to `AdminLayout.astro` (hidden on `/admin/login`). Verified 2026-06-14 — see `.claude/plans/22-Admin-Dashboard-F15.md`.
- **F-16** ✓ Admin job list — `src/pages/admin/jobs/index.astro`. All jobs (live + draft + expired), colour-coded status badges (Live=green, Draft=amber, Expired=red), 50/page with inline pagination, flash banners for post-action redirects, inline delete confirm. Verified 2026-06-14 — see `.claude/plans/23-Admin-Jobs-List-F16.md`.
- **F-17** ✓ Post new job — `src/pages/admin/jobs/new.astro`. 6-section form; state required for Govt/PSU; auto-slug from title with uniqueness suffixing; KV invalidated on insert. Verified 2026-06-14 — see `.claude/plans/24-Post-New-Job-F17.md`.
- **F-18** ✓ Edit job — `src/pages/admin/jobs/[id]/edit.astro`. Pre-populated 6-section form; same validation as F-17; `makeUniqueSlugExcluding()` skips uniqueness check when slug unchanged; `UPDATE jobs SET … updated_at = CURRENT_TIMESTAMP`; inline slug-change warning (orange, `#f5a623`); KV invalidated; redirect to `/admin/jobs?updated=1`. Build verified 2026-06-14.
- **F-19** ✓ Delete job — `src/pages/admin/jobs/[id]/delete.astro`. POST-only; 405 on GET; hard `DELETE FROM jobs WHERE id = ?`; KV invalidated; redirect to `/admin/jobs?deleted=1`. Build verified 2026-06-14.
